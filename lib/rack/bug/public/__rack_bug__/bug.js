...
jQuery(function($) {
  ...
  $.extend($.rackBug, {
    init: function() {
      var current = null;
      var cookie = document.cookie.match(/rack_bug_location=([^;]+)/);
      $("#rack_bug").removeClass("rb_top")
                    .removeClass("rb_bottom")
                    .addClass("rb_" + ((cookie && (cookie[1] == 'bottom')) ? 'bottom' : 'top'));
      ...
      $('#rb_debug_button').click(function(){
        ...
        var loc = $('#rack_bug').attr("class").match(/rb_(top|bottom)/);
        if( loc ) {
          document.cookie = "rack_bug_location=" + loc[1] + "; path=/"
        }
        return false;
      });
